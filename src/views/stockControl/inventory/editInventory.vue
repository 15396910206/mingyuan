            ...
            <div>
                ...
                >
                    ...
                    <el-table-column prop="name" label="名称" width="200" show-overflow-tooltip>
                        <template slot-scope="scope">
                            ...
                            <el-select
                                    ...
                                    placeholder="请选择"
                                    @change="productChange(scope.$index,scope.row)"
                            >
                                <el-option
                    ...
                    <el-table-column prop="oldBalance" label="盘点数量">
                        <template slot-scope="scope">
                            <el-input
                                    v-model="scope.row.oldBalance"
                                    type="number"
                                    min="0"
                                    placeholder="请输入"
                                    @change="numberChange(scope.$index,scope.row)"
                                    v-enter-number
                            ></el-input>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="remark" label="处理说明">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.remark" placeholder="请输入处理说明"></el-input>
                        </template>
                    </el-table-column>
                    <el-table-column label="出入库单据">
                        <template slot-scope="scope">
                            <el-button type="primary" size="mini" @click="showDetailed(scope.row)">出入库明细</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                pickerOptions: {
                    disabledDate(time) {
                        return (
                            time.getTime() < Date.now() - 8.64e7 * 6 ||
                            time.getTime() > Date.now() + 8.64e7 * 6
                        );
                    },
                }, //时间配置
        ...
        methods: {
            ...
            dateScope: function () {
                var that = this;
                API.getDateScope().then((res) => {
                    if (res.stateCode == 100) {
                        var startDay = Math.floor(
                            (new Date().getTime() - res.data.AccountDate) / 86400000
                        );
                        var endDay = Math.floor(
                            (new Date().getTime() - res.data.MaxDate) / 86400000
                        );
                        that.defaultValue = new Date(res.data.AccountDate + 86400000);
                        that.pickerOptions = {
                            disabledDate(time) {
                                return (
                                    time.getTime() < Date.now() - 8.64e7 * startDay ||
                                    time.getTime() > Date.now() - 8.64e7 * endDay
                                );
                            },
                        };
                    } else {
            ...
            stockData: function () {
                ...
                that.productList = [];
                API.selectNumberByList(params).then((res) => {
                    if (res.stateCode == 100) {
                        ...
                        that.replenishmentList.push.apply(
                            that.replenishmentList,
            ...
            productData: function () {
                ...
                API.codeProductList(params).then((res) => {
                    if (res.stateCode == 100) {
            ...
            // 新增商品
            addProduct: function (index, row) {
                var that = this;
                that.zeroDisplay = false;
                that.selectPage = 1;
                that.productList = [];
                if (
                    that.ruleForm.warehouseID == "" ||
                    that.ruleForm.warehouseID == null
                ) {
                    that.$message.error({
                        message: "请选择盘点仓库",
                        duration: 2000,
            ...
            priceDetailData: function (index) {
                ...
                API.codeProductDetail(params).then((res) => {
                    if (res.stateCode == 100) {
                        ...
                        that.replenishmentList[index].Specification = res.data.Specification;
                        if (
                            that.replenishmentList[index].oldBalance >
                            that.replenishmentList[index].amount
                        ) {
